...
public class DAO_producto {
    ...
    private String sql_insert = "INSERT INTO producto(idproducto,cod_barra,nombre,precio_venta_minorista,precio_venta_mayorista,"
            ...
            + "precio_alquiler,precio_reposicion,descripcion) "
            + "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);";
    ...
    private String sql_cargar = "SELECT p.idproducto,p.cod_barra,p.nombre,p.precio_venta_minorista,p.precio_venta_mayorista,p.cantidad_mayorista,p.precio_compra,\n"
            ...
            + "p.precio_alquiler,p.precio_reposicion,p.descripcion \n"
            + "FROM producto p,producto_unidad pu,producto_categoria pc, producto_marca pm \n"
            + "where p.fk_idproducto_unidad=pu.idproducto_unidad\n"
            + "and p.fk_idproducto_categoria=pc.idproducto_categoria\n"
            + "and p.fk_idproducto_marca=pm.idproducto_marca\n"
    ...
    private String sql_stock_minimo = "SELECT p.idproducto as idp,\n"
            ...
            + "and p.activar=true\n"
            + "order by 4 desc";
    private String sql_stock_devolucion = "update producto set stock=(stock+"
            + "(select sum(iva.cantidad_total) as cant from item_venta_alquiler iva "
            + "where iva.fk_idproducto=iva2.fk_idproducto "
            + "and iva.fk_idventa_alquiler=iva2.fk_idventa_alquiler)) \n"
            + "from item_venta_alquiler iva2 \n"
            + "where producto.idproducto=iva2.fk_idproducto\n"
            + "and iva2.fk_idventa_alquiler=";
    private String sql_stock_alquilado = "update producto set stock=(stock-"
            + "(select sum(iva.cantidad_total) as cant from item_venta_alquiler iva "
            + "where iva.fk_idproducto=iva2.fk_idproducto "
            + "and iva.fk_idventa_alquiler=iva2.fk_idventa_alquiler)) \n"
            + "from item_venta_alquiler iva2 \n"
    ...
    public void insertar_producto(Connection conn, producto pro) {
        ...
        try {
            ...
            pst.setDouble(21, pro.getC24precio_alquiler());
            pst.setDouble(22, pro.getC25precio_reposicion());
            pst.setString(23, pro.getC26descripcion());
            pst.execute();
            pst.close();
    ...
    public void update_producto(Connection conn, producto pro) {
        ...
        try {
            ...
            pst.setDouble(20, pro.getC24precio_alquiler());
            pst.setDouble(21, pro.getC25precio_reposicion());
            pst.setString(22, pro.getC26descripcion());
            pst.setInt(23, pro.getC1idproducto());
            pst.execute();
            pst.close();
    ...
    public void cargar_producto_por_idproducto(Connection conn, producto pro, int id) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                pro.setC24precio_alquiler(rs.getDouble(24));
                pro.setC25precio_reposicion(rs.getDouble(25));
                pro.setC26descripcion(rs.getString(26));
    ...
    public void ancho_tabla_producto(JTable tbltabla) {
        int Ancho[] = {5, 7, 12, 5, 10, 24, 6, 6, 7, 7, 4, 4, 4};
    ...
    public void buscar_tabla_producto(Connection conn, JTable tbltabla, String filtro) {
        String sql = "select p.idproducto as idp,p.cod_barra as barra,\n"
                + "pc.nombre as categoria,pu.nombre as unid,pm.nombre as marca,\n"
                + "p.nombre as nombre,\n"
                + "TRIM(to_char(p.precio_venta_minorista,'999G999G999')) as pventa,\n"
                + "TRIM(to_char(p.precio_compra,'999G999G999')) as pcompra,\n"
                + "TRIM(to_char(p.precio_alquiler,'999G999G999')) as palquiler,\n"
                + "TRIM(to_char(p.precio_reposicion,'999G999G999')) as preposicion,\n"
                + "p.stock_fijo as sfijo,p.stock as stock,p.activar \n"
