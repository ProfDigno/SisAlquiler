 ...
 * @author Digno
 */
public class FrmProducto_marca extends javax.swing.JInternalFrame {

    EvenJFRAME evetbl = new EvenJFRAME();
    EvenJtable evejta = new EvenJtable();
    producto_marca cate = new producto_marca();
    BO_producto_marca pcBO = new BO_producto_marca();
    DAO_producto_marca pcdao = new DAO_producto_marca();
    ...
    void actualizar_tabla_categoria() {
        if ((txtfecha_desde.getText().trim().length() > 0) && (txtfecha_hasta.getText().trim().length() > 0)) {
            ...
            pcdao.actualizar_tabla_producto_marca(conn, tblpro_categoria, fecdesde, fechasta);
        }
    }
    ...
    void boton_guardar() {
        if (validar_guardar()) {
            cate.setC2nombre(txtnombre.getText());
            cate.setC3activar(jCactivar.isSelected());
            cate.setC4orden(Integer.parseInt(txtorden.getText()));
            pcBO.insertar_producto_marca(cate, tblpro_categoria);
            reestableser();
        }
    }
    ...
    void boton_editar() {
        if (validar_guardar()) {
            cate.setC1idproducto_marca(Integer.parseInt(txtid.getText()));
            cate.setC2nombre(txtnombre.getText());
            cate.setC3activar(jCactivar.isSelected());
            cate.setC4orden(Integer.parseInt(txtorden.getText()));
            pcBO.update_producto_marca(cate, tblpro_categoria);
            actualizar_tabla_categoria();
        }
    }

    void seleccionar_tabla() {
        pcdao.cargar_producto_marca(conn, cate, tblpro_categoria);
        txtid.setText(String.valueOf(cate.getC1idproducto_marca()));
        txtnombre.setText(cate.getC2nombre());
        jCactivar.setSelected(cate.getC3activar());
        txtorden.setText(String.valueOf(cate.getC4orden()));
...
//                pdao.actualizar_tabla_producto_en_grupo(conn, tblpro_producto, fechadesde, fechahasta, idproducto_categoria);
    ...
    private void initComponents() {
        ...
        jTabbedPane1.addTab("CREAR MARCA", jPanel3);
    ...
    private void formInternalFrameOpened(javax.swing.event.InternalFrameEvent evt) {//GEN-FIRST:event_formInternalFrameOpened
        // TODO add your handling code here:
        pcdao.ancho_tabla_producto_marca(tblpro_categoria);
